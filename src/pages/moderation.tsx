...
const returnFlag = (frame, duration) => {
  ...
  switch (frame.pornographyLikelihood) {
    case "POSSIBLE":
      return (
        <div
          key={frame.timeOffset}
          className="bg-yellow-300 w-1 h-8 absolute -top-2 rounded"
          style={{
            left: `${position}%`,
          }}
        ></div>
    ...
    case "LIKELY":
      return (
        <div
          key={frame.timeOffset}
          className="bg-orange-500 w-1 h-8 absolute -top-2"
          style={{
            left: `${position}%`,
          }}
        ></div>
    ...
    case "VERY_LIKELY":
      return (
        <div
          key={frame.timeOffset}
          className="bg-red-600 w-1 h-8 absolute -top-2"
          style={{
            left: `${position}%`,
          }}
        ></div>
    ...
    default:
      return (
        <div
          key={frame.timeOffset}
          className="bg-green-200/10 w-1 h-4 absolute top-0"
          style={{
            left: `${position}%`,
          }}
        ></div>
        ...
        <div className="flex flex-col w-full">
          ...
          <ul className="flex flex-row flex-wrap pl-0 mb-4 list-none border-b-0 nav nav-tabs md:flex-row">
            <li className="nav-item">
              <button
                className={`block px-6 py-3 my-2 border-b-4 border-transparent text-xs font-medium leading-tight uppercase hover:bg-secondary-color ${
                  activeTab === "player"
          ...
          {activeTab == "activeModerators" && (
            <div className="flex flex-col max-w-7xl">
              {data.moderators.active.map((moderator, id) => (
                <div
                  className="flex items-center justify-between pr-4 mb-4 bg-primary-color rounded-lg"
                  key={id}
                >
                  <div className="w-80 hover:text-gray-300 cursor-pointer relative">
                    <div className="w-full h-40 bg-black flex justify-center rounded">
                      <img src={moderator.avatar} className="h-full" />
                    </div>
                    <div className="w-full h-40 flex justify-center rounded absolute left-0 top-0 bg-black opacity-0 hover:opacity-50"></div>
                  </div>
              ...
              {data.moderators.pending.map((moderator, id) => (
                ...
                >
                  ...
                  <div className="ml-2 px-2">
                    <div>
                      <button
                        className="w-24 text-lg text-center dm-sans bg-green-400 rounded-lg"
                        onClick={() => {
          ...
          {activeTab == "videosModeration" && (
            <div className="flex flex-col max-w-7xl">
              {data.videos.map((video, id) => (
                <div
                  key={id}
                  className="flex items-center justify-between pr-4 mb-4 bg-primary-color rounded-lg relative"
                >
                  <div className="w-80 hover:text-gray-300 cursor-pointer relative">
                    <div className="w-full h-40 bg-black flex justify-center rounded">
                      <img src={video.poster} className="h-full" />
                    </div>
                    <div className="w-full h-40 flex justify-center rounded absolute left-0 top-0 bg-black opacity-0 hover:opacity-50"></div>
                  </div>
                  <div className="ml-2 px-2">
                  ...
                  <div className="ml-2 px-2">
                    <div className="flex space-x-6">
                      <button
                        className="w-24 text-lg text-center dm-sans bg-red-400 hover:bg-red-500 rounded-lg"
                      ...
                      <button
                        className="w-24 text-lg text-center dm-sans bg-green-400 hover:bg-green-500 rounded-lg"
                        onClick={() => {
                          signAsHash(["string"], ["abc123"])
                            .then(console.info)
                            .catch(alert);
          ...
          <div className="bottom-0 right-0 items-center w-full py-4 mt-4 px-4 border-gray-200 max-w-7xl sm:flex sm:justify-between text-white">
            <div className="flex items-center mb-4 sm:mb-0">
              <span className="text-sm font-normal text-gray-100">
                Showing <span className="font-semibold text-white">1-20</span>{" "}
                of <span className="font-semibold text-white">2290</span>
              </span>
            </div>
